# 👋 Assalomu alaykum! Men Doniyorbek

💻 **Full-Stack & Backend Dasturchi | Python Backend Developer**  
🎓 **Zomin IT Park – Python Backend Sertifikati (2025)**  
🎯 Maqsad: Professional dasturchi bo‘lish va zamonaviy, foydalanuvchi uchun qulay web ilovalar yaratish  

---

## 🌟 Men haqimda qisqacha
- 🔧 **Backend:** Python, Django, FastAPI asosida zamonaviy web-ilovalar yarataman  
- 📱 **Frontend:** React bilan integratsiyalashgan, responsiv platformalar ishlab chiqaman  
- 🚀 **API & NLP:** REST API va NLP loyihalarini boshqaraman  
- 👨‍💻 Har doim yangi texnologiyalarni o‘rganishga va tajriba almashishga tayyorman  
- 🏗️ **Loyiha tajribasi:** 5+ kichik va o‘rta web-loyihalar yaratganman  

---

## 🏆 Sertifikatlar
- 🎓 **Python Backend Development** – Zomin IT Park, 2025
- 📄 [PDF nusxasi](https://uzcorp-certificates.uz/certificate/801/)

---

## 💻 Texnologiyalar va Asboblar
**Dasturlash tillari:** `Python`, `JavaScript`, `HTML`, `CSS`  
**Backend:** `Django`, `FastAPI`, `REST API`  
**Frontend:** `React`, `Tailwind CSS`, `Bootstrap`  
**Ma’lumotlar bazasi:** `MySQL`, `PostgreSQL`, `SQLite`  
**Asboblar:** `Git`, `GitHub`, `Docker`, `VS Code`, `Linux`  

---

## 💻 “Kod - bu fikringni dunyoga ko'rsatish usuli !”
<table>
  <tr>
    <td><img src="https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExZmIwZzFrdnQ5MDQ5NDR0ZW03OGpnd2pmNGttOXZodnlyZmk4c3NodyZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/26tn33aiTi1jkl6H6/giphy.gif" width="200" alt="Kod yozish 1"></td>
    <td><img src="https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExeXRsbjFyZGs4bGx6dXAwY21hc29zdGkwY21iM3B5dWI2eXBpeTEzMCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/3NE7JhJgZBHlMfmNEa/giphy.gif" width="200" alt="Kod yozish 2"></td>
    <td><img
scr="https://media4.giphy.com/media/v1.Y2lkPTc5MGI3NjExeGRmYmppMGZwYWJueHZvd2dmazl3Ymp3d29pYWd0OWJjM2tteTc4ZCZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/p4NLw3I4U0idi/giphy.gif"  alt="Kod yozish 3"></td>          
  </tr>
</table>


---
## 📊 Men biladigan tillar 
![O'zbek](https://camo.githubusercontent.com/14d82665a9ad8800d1fb31c282370618ae102d8cb8c9f0922a1862a9b39376cf/68747470733a2f2f666c616763646e2e636f6d2f33327832342f757a2e706e67)O'zbek
![Ingliz](https://camo.githubusercontent.com/9bbaaf9cb00bb4ced0f978dd83f2674e9c6ee3cc7f0108a80f8b3233a021d4e7/68747470733a2f2f666c616763646e2e636f6d2f33327832342f67622e706e67)Ingliz



---
## 🚀 Loyihalarim

### [Rassom AI Bot](https://t.me/Rassom_Chizadigan_AI_bot)
Zamonaviy AI yordamida rasm chizadigan Telegram boti.  

![Rassom AI Bot](https://media2.giphy.com/media/v1.Y2lkPTc5MGI3NjExbW9paTA0aDExMGczZGc2MDJnbzZxZThiZmFvYWpnY2lweDJvdmVneSZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/cCeXvYeTLKlmU/giphy.gif)

---

### [Portfolio Web App](https://github.com/Robotech203/portfolio-web)
Shaxsiy portfolio web-sayti. HTML, CSS va React bilan yaratilgan.  

![Portfolio Preview](https://wiztoonz.com/wp-content/uploads/2022/04/Blog-Post-Portfolio-1170x658.jpg)

---

### [Telegram ChatGPT Bot](https://t.me/Uz_ChatGPT_AI_bot)
Telegram orqali ishlaydigan ChatGPT bot. Foydalanuvchi bilan interaktiv suhbatlar.  

![ChatGPT Bot](https://media0.giphy.com/media/v1.Y2lkPTc5MGI3NjExbnJsZHkyeXJvb212bzZqaGR5NjRkbHZuNGwxNWttd2JvMjJmZWQ3ayZlcD12MV9pbnRlcm5hbF9naWZfYnlfaWQmY3Q9Zw/f0VbswUmSY6X1cBz4C/giphy.gif)


---

## 📬 Men bilan bog‘laning
- 🌐 GitHub: [Robotech203](https://github.com/Robotech203)  
- 💬 Telegram: [@doniyorbek_coder](https://t.me/doniyorbek_coder)  
- ✉️ Email: coderdoniyorbek@gmail.com  
- 🌐 LinkedIn: [Doniyorbek](https://www.linkedin.com/in/doniyorbek/)  
- 🌐 Portfolio: [Portfolio Web App](https://github.com/Robotech203/portfolio-web)  

---

## ✨ Shaxsiy maqsad va qiziqishlar
Yosh dasturchi, **15 yosh**, Zomin — **web va robototexnikaga qiziqaman**.  
Maqsadim: **Professional dasturchi bo‘lish va foydalanuvchi uchun qulay, zamonaviy dasturiy ta’minot yaratish.**  
🚀 Kelajakda Python va Full-Stack texnologiyalar asosida innovatsion loyihalarni amalga oshirishni maqsad qilganman.  
🔧 Qiziqishlar: web dasturlash, backend, frontend, AI/NLP, robototexnika, zamonaviy web-loyihalar  

---


